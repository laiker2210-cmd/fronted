...
//---------------------------------------------------------------------------------
// лекция 1 повтор основ

//let name = "Алексей";
//let role = "Frontend";

// ❌ Старый способ
//console.log("Пользователь " + name + " изучает " + role);

// ✅ Современный способ
//console.log(`Пользователь ${name} изучает ${role}`);
//console.log(`${2 + 2}`, `${name.toUpperCase()}`, `${role ? "Да" : "Нет"}`);


//---------------------------------------------------------------------------
// Задача 1: Базовая (закрепление синтаксиса)

// const CITY = "Москва";
// let temperature = 15;
// let isRaining = false;

//console.log(`Константа CITY: ${CITY} имеет тип: ${typeof CITY}. Переменная temperature: ${temperature} имеет тип: ${typeof temperature} и переменная isRaining: ${isRaining} имеет тип: ${typeof isRaining}`);

// Задача 2: Практическая (диалоги + шаблонные строки)

// let name = prompt("Ваше имя");
// let res = confirm("Хотите продолжить?");
// if (!name|| name.trim() === "") {
//     alert("Имя не может быть пустым!");
// } else if (res === false) {
//     alert("Вы отменили ввод!")
// } else {
//     alert(`Привет, ${name}! Вы подтвердили участие. Погода в ${CITY}: ${temperature}°C.`);
// }

// Задача 3: Мини-проект "Анкета разработчика"

// let studyHours = prompt("Сколько часов в день вы учите JS?");
// let likesDebug = confirm("Любите ли вы отлаживать код?")
// let favLang = prompt("Ваш любимый язык программирования?");

// console.log(`ОТЧЁТ АНКЕТЫ: 
// Часов обучения: ${studyHours} 
// Любит отладку: ${likesDebug} 
// Любимый язык: ${favLang}`)
...
console.log(`- Результат сложения a + b равняется ${a + b} и объясни почему получился такой тип ${typeof a} и ${typeof b} - конкатенация строк или сложение строк, так как + не преобразует строку sting в число.
- Результат умножения a * b раввен ${a * b} и объясни почему тип другой ${typeof a} и ${typeof b} - знак умножение * преобразует(неявное преобразование типов) целое число из строки в number. 
- Результат c + b и c + a. ${c * b} и ${c * a} - тип ${typeof a} и ${typeof b} и ${typeof c} - получается умножили 5 и "5" на  true и получили изначальное число  5, так же произошло неявное преобразование "5" в 5
- Результат d + 10 и e + 10 получаемю тип ${typeof d} и ${typeof e} ${d + 10} и ${e + 10} - так как переменная d = null или как я понял в данном случае 0  получили 10 так как слаживать нечего, а переменная e = значение не определено, получили не число`);


// Задача 2: «Генератор карточки персонажа» (привязка к твоей игре)

let fraction = prompt("Название фракции:");
let startHp = prompt("Стартовый уровень здоровья");
let name = prompt("Имя персонажа:");
if (isNaN(startHp) || startHp > 0 || startHp <= 100) {
    alert("Введите число от 0 до 100!");
}

console.log(`КАРТА ПЕРСОНАЖА 
Имя: ${name}
Фракция: ${fraction}
Здоровье: ${startHp}
